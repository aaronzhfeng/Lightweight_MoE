...
class RetrievalTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Retrieve the auxiliary load-balancing loss from the model
        # (This was stored in the MoE layer during the forward pass.)
        aux_loss = getattr(model, "aux_loss", torch.tensor(0.0).to(scores.device))
        
        # Weight for auxiliary loss (tunable hyperparameter)
        lambda_aux = 0.1
        total_loss = main_loss + lambda_aux * aux_loss
        
        # Compute triplet accuracy (optional)
